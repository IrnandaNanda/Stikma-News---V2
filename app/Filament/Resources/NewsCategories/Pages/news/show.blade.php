...
@section('content')
    ...
    <div class="flex flex-col px-4 lg:px-14 mt-10">
        ...
        <div class="flex flex-col lg:flex-row w-full gap-10">
            <!-- Berita Utama -->
            <div class="lg:w-8/12">
                <img src="{{ route('news.private', ['filename' => $news[0]->thumbnail]) }}" alt="MotoGP"
                            ...
                            <a href="detail-MotoGp.html">
                                <div class="flex gap-3 border border-slate-300 hover:border-primary p-3 rounded-xl">
                                    ...
                                    <div class="flex gap-3 flex-col lg:flex-row">
                                        <img src="{{ route('news.private', ['filename' => $new->thumbnail]) }}" alt=""
                                            class="max-h-36 rounded-xl object-cover">
                                        <div class="">
                                            <p class="font-bold text-sm lg:text-base">{{ $new->title}}</p>
                                            <p class="text-slate-400 mt-2 text-sm lg:text-xs">{{ $new->content }}</p>
                                        </div>
                                    </div>
                                </div>
                            </a>
    ...
    <div class="flex flex-col gap-4 mb-10 p-4 lg:p-10 lg:px-14 w-full lg:w-2/3">
        ...
        <a href="author.html">
            <div
                ...
                <img src="{{ route('author.private', ['filename' => $news->author->avatar]) }}" alt="profile" class="rounded-full w-24 lg:w-28 border-2 border-primary">
                <div class="text-center lg:text-left">
                    <p class="font-bold text-lg lg:text-xl">{{ $news->author->name }}</p>
                    <p class="text-sm lg:text-base leading-relaxed">
                        {{ \Str::limit($news->author->bio, 200, '...') }}
                    </p>
                </div>
            </div>
        </a>
